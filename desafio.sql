...
CREATE DATABASE desafio3_yaritza_navarrete_001;
CREATE TABLE users(
  id SERIAL,
...
SELECT MAX(posts.updated_at), users.name 
...
ORDER BY (SELECT COUNT(*) FROM comments WHERE comments.post_id = posts.id) DESC LIMIT 1;
...
SELECT 
  ...
  comments.content 
FROM comments
INNER JOIN (SELECT MAX(comments.id) AS comment_id FROM comments GROUP BY user_id) AS max_comment
ON comments.id = max_comment.comment_id
ORDER BY comments.user_id;


10.Muestra los emails de los usuarios que no han escrito ningún comentario. 

SELECT users.email FROM users LEFT JOIN comments ON comments.user_id = users.id WHERE comments.id IS NULL;